#!/usr/bin/env python3
"""Rewrite Thai translations in fellows-db.json with proper natural Thai."""

import json

# Dictionary of corrected Thai translations keyed by fellow name + field
# Only including fields that have meaningful Thai to fix

CORRECTIONS = {

    # === DEMIGOD FELLOWS ===

    "Inferno Zaura|leaderSkill": "Darkest Blaze Fire และ Dark element fellows (ยกเว้น Demigod grade) ignore defense +1. 25% x rank",
    ...
    "Little Star Candy Vakarine|leaderSkill": "ดาว Point ทุกครั้งที่ Ice element fellows (ยกเว้นตัวนี้) ใช้สกิล ให้ Star Point เท่ากับครึ่งหนึ่งของ rank ของ fellow นั้น (Max 100 stacks) Star Point จะลดลง 1 stack ทุก 2 วินาที และฮีลผู้เล่น 2% HP และ 1% SP เมื่อ stack ถึงสูงสุด ให้ buff 15s แก่ผู้เล่นและทุก fellows ที่เพิ่ม damage 7.5% (ยกเว้น Demigod Fellow) ไม่สามารถ stack Star Point ขณะที่ buff นี้ทำงานอยู่",
    ...
    "Revenger Hauberk|leaderSkill": "วิญญาณ Fragment เพิ่มโจมตีให้ทุก fellows ในทีม (ยกเว้น Demigod Fellow) 0.85% x rank ของตัวนี้ เมื่อ fellow คนอื่นถูกทำให้หมดสติ ดูดซับ 100% ของ HP ของ fellow นั้นเพื่อฮีลตัวเองและเข้าสู่สถานะ Revenge 45s เพิ่มโจมตีตัวเอง 50% (trigger passive นี้อีกครั้งจะรีเฟรชระยะเวลา)",
    ...
    "Revenger Hauberk|skill3": "Venge of Souls CD 20s tấn công liên tục bằng vuốt. สร้างความเสียหาย 427% x 7 ยิ่งโจมตีความเร็วสูง ยิ่ง cast ความเร็วของสกิลนี้สูงขึ้นด้วย",
    ...
    "Champion Gabija|leaderSkill": "Immortality Fire และ Holy element fellows (ยกเว้น Demigod fellows) ลด damage ที่รับ 2% x rank ของตัวละครนี้ effect นี้ไม่ทำงานเมื่อตัวนี้ถูกทำให้หมดสติ พร้อมให้ตัวนี้ +5% block Rate ต่อ unit ที่ได้รับ buff นี้ สูงสุด 25% เพิ่ม",
    ...
    "Gumiho Lada|leaderSkill": "Allure of Gumiho ให้ Tail Blessing แก่ทุก Lightning และ Multi-element fellows (ยกเว้น Demigod Fellows) ขณะที่ blessing ทำงาน fellow ที่มี blessing จะเสีย 5% HP ทุก 5s แล้วฮีล master เท่ากับจำนวน HP ที่เสีย พร้อมลด damage จาก Ice-element ศัตรู 35% และเพิ่ม Lightning damage (ยกเว้น DoT) 9% x rank ของตัวละครนี้ ทุกโจมตีจาก fellow นี้และ fellows ที่ได้รับ blessing จะให้ 1 Spark stack แก่ศัตรูที่โดน",
    ...
    "Holy Knight Arlene|leaderSkill": "Subduer ให้ buff แก่ตัวเองและทุก fellows อื่น (ยกเว้น Demigod fellow) ลด damage จาก boss และ Demigod ตัวละคร 25% โบนัสนี้ลดลง 5% ต่อ Demigod fellow อื่นในทีม เมื่อต่อสู้กับ boss หรือ Demigod ตัวละคร จะให้โบนัส effect พื้นฐานบน tree ของ fellow: Swordman/Cleric: เพิ่ม block Rate 5% x ศักดิ์สิทธิ์ Knight rank Wizard: เพิ่ม penetration Rate 5% x ศักดิ์สิทธิ์ Knight rank Archer/Gunner: เพิ่ม hit Rate 5% x ศักดิ์สิทธิ์ Knight rank Scout: เพิ่ม evasion Rate 5% x ศักดิ์สิทธิ์ Knight rank",
    "Holy Knight Arlene|skill1": "ศักดิ์สิทธิ์ Touch CD 15s ปล่อยแสงศักดิ์สิทธิ์ชำระล้างรอบตัว สร้างความเสียหาย 913% และลบ debuffs ทั้งหมดจาก allies ทั้งหมด ให้ตัวเอง stacks ของ ศักดิ์สิทธิ์ Knight เท่ากับจำนวน ally ที่ได้รับผลกระทบจากสกิลนี้ [ศักดิ์สิทธิ์ Knight] buff ถาวร ลด damage จาก boss และ Demigod ตัวละคร 1% ต่อ stack สูงสุด 45 stacks",
    ...
    "Holy Knight Arlene|awakenSkill": "Trinity [Unlock ที่ rank 1] CD 120s เข้าสู่สถานะ Awakening 30s และให้ 5 stacks ของ ศักดิ์สิทธิ์ Knight, Purifier และ Slayer [Awakening] Knockback Immune ทุกโจมตีสร้างความเสียหายเพิ่มเติม 1.5% ของ Max HP ของศัตรู (damage จาก Max HP จำกัดที่ 20% ของโจมตี power ของตัวนี้) เมื่อถูกโจมตีโดย boss หรือ demigod ตัวละคร ลด cooldown ของสกิลนี้ 10s",
    ...
    "Sweet Gothic Nebulas|leaderSkill": "ผี Sharing ให้ตัวนี้ minimum ของ Gothic ผี และ ผี buff ที่ขึ้นกับว่ามี fellows ปกติกี่ตัวในทีม - Gothic ผี: จำนวน fellows ปกติ (สูงสุด 5 stacks) - ผี: ผลรวมของ rank ของ fellows ปกติ (สูงสุด 100 stacks) เมื่อ demigod fellow ในทีมถูกทำให้หมดสติ ให้ master สถานะ Revenge 10s (CD 60s) [Revenge] - เพิ่ม movement และโจมตีความเร็ว 20% พร้อม immune ต่อช้า - Endurance +20% - พื้นฐานโจมตีลด cooldown ของทุกสกิล 0.5s",
    ...
    "Milky Way Starlight Jurate|leaderSkill": "Milky Way เมื่อตัวนี้ใช้สกิล activate [Resonance Effect] บน master และทุก Ice-Element fellows ในทีม (ยกเว้น Demigod Fellows)",
    ...
    "Milky Way Starlight Jurate|tierlist_notes": "SS God of Water แรงมากเมื่อเจอศัตรู Fire ใดๆ แต่ก็แค่นั้นแหละกับทุกอย่างอื่น เธอทำ damage ดีหลัง unlock rank 5-6 ขณะให้ mitigation จาก rank 4 ทำให้เธอค่อนข้าง versatile แม้ไม่ได้เจอศัตรู Fire",

    "Iron Fan Marnox|leaderSkill": "Breeze เมื่อ Marnox หลบโจมตีของศัตรูสำเร็จ ให้ buff Breeze แก่ตัวเอง, เจ้าของ และ non-demigod Archer/Gunner/Scout fellows 5s [Breeze] คงอยู่ 5s เพิ่มโจมตีและ movement ความเร็ว 10% เมื่อระยะเวลาสิ้นสุด ฮีลทุกตัวที่ได้รับผลกระทบจาก buff นี้เท่ากับจำนวน evasion ของ Marnox (สูงสุด 20% ของ Max HP ของผู้รับ) พร้อมคืน 5% SP แก่เจ้าของ",
    ...
    "Amberscale Velcoffer|leaderSkill": "Flamescale เพิ่ม defense ของทุก fellows (ยกเว้น Demigod grade) 2.5% x rank ของตัวละครนี้ ใช้ Hush Veil Skill ให้ Demigod fellows 10% defense 25s",
    ...
    "[TIP] Aura Black|leaderSkill": "[TIP] (TOS Idol Project) Idol เมื่อทีมมีคอมโบของ fellows ที่มี element เหล่านี้ ให้ทุก fellows ผลของ effect ต่อไปนี้: - Fire + Ice: ทุกโจมตีมี 10% โอกาสให้ Heron Heron (Mental-type debuff) แก่ศัตรู ลดโจมตี 3.5% 10s - Ice + Lightning: ทุกโจมตีมี 10% โอกาสให้ Surprise (Physical-type debuff) แก่ศัตรู ลด Hit และ Penetration value 5% 10s - Fire + Lightning: ทุกโจมตีมี 10% โอกาสให้ Dazzle (Magical-type debuff) แก่ศัตรู เพิ่ม damage ที่รับ 5% 10s",
    "[TIP] Aura Black|skill1": "แสดง CD 7s [สามารถใช้ได้เฉพาะเมื่อมี 10 stacks ของ Performance] เริ่มแสดง 10s มีผลต่อทุก allies ใกล้เคียง ทุก 2s ฮีลทุก allies ที่ไม่ได้รับผลกระทบจากสกิลฮีลของตัวนี้ 25% ของ Healing power ลด damage taken 50% ขณะแสดง",
    "[TIP] Aura Black|awakenSkill": "บน Stage [Unlock ที่ rank 1] CD 120s เข้าสู่สถานะ Awakening 30s [Awakening - บน Stage] - ตลอดการแสดง ฮีลทุก allies ทุก 2s",

    "Noble Giltine|leaderSkill": "Fate Healer ทุก fellows (ยกเว้น Demigod fellows) ลด damage ที่รับจากปีศาจ-type, มนุษย์-type และ Demigod-type ศัตรู 1.5% x rank ของตัวนี้ + 2% x rank ของ unit นั้นๆ Giltine เองลด damage ที่รับจาก type ที่ตรงกันของศัตรู 8% x unit ที่ได้รับผลกระทบเหนือ effect (สูงสุด 40%) และเพิ่มโจมตีเมื่อโจมตีศัตรูเหล่านั้น 20% ต่อ Demigod ตัวละครในทีม (รวมตัวเอง สูงสุด 80%)",
    ...
    "Night Prophet Laima|leaderSkill": "Zodiac ทุก 30s cycle buff ไปหนึ่งในประเภทของ fellow (Demigod -> Archer/Gunner -> Wizard -> Swordman และ Rider -> Scout -> Cleric ข้ามไปตัวถัดไปถ้าประเภทนั้นไม่มีในทีม) ที่เพิ่มโจมตี 12% ระยะเวลาเท่ากับผลรวมของ rank ของ fellows จากประเภทนั้น (สูงสุด 30s) และยัง stack buff นี้แก่เจ้าของเท่ากับจำนวน fellows จากแต่ละ element ในทีม - Fire - Mars Gabija: เพิ่ม Fire damage 3% - น้ำ - Mercury Jurate: เพิ่ม Ice damage 3% - ดิน - Saturn Zemyna: เพิ่ม Earth damage 3% - สายฟ้า - Jupiter Rexipher: เพิ่ม Lightning damage 3% - ศักดิ์สิทธิ์ - Solar Saule: เพิ่ม Holy damage 3% - มืด - Lunar Naktis: เพิ่ม Dark damage 3% - Multi - Rainbow Lada: ลด damage taken 1%",
    ...
    "Penguin Queen Austeja|leaderSkill": "Pioneer เพิ่ม Max HP ให้เจ้าของและ Cleric-type fellow ในทีม 20% (10% PvP) ทุก 2s ฟื้นฟู HP ให้ unit เหล่านั้น: เจ้าของ: 12% ของ HP ที่เสีย Cleric fellows: 2x Rank% ของ HP ที่เสีย",
    ...
    "Dawn Raven Rexipher|leaderSkill": "End of Dawn เมื่อ Rexipher ใช้สกิล Awakening เรียกเงาของ Earth และ Multi-Element fellows ในทีมยกเว้นตัวเอง (สูงสุด 3) ศัตรูใกล้เงาได้รับ debuff Dawn ที่ stack ได้ เพิ่ม damage ที่รับ 4% เงามี HP 100% ของ fellow ที่เรียกมา และตามศัตรูใกล้เคียงโดยไม่โจมตี",
    ...
    "Fortune Bunny Girl Lucy|leaderSkill": "Fever เวลา ให้เจ้าของและทุก fellows +25 Crit Rate ต่อตัวละคร level Critical Hit damage ต่อศัตรูในสถานะ control effect เพิ่ม 333%",
...
def apply_corrections(data):
    """Apply the corrections dictionary to the data."""
    corrections_applied = 0
    for fellow in data:
        name = fellow.get('name', '')
        for key in ['leaderSkill', 'basicAttack', 'skill1', 'skill2', 'skill3', 'awakenSkill']:
            if key in fellow and fellow[key]:
                dict_key = f"{name}|{key}"
                if dict_key in CORRECTIONS:
        ...
        if 'tierlist' in fellow and fellow['tierlist'] and 'notes' in fellow['tierlist']:
            ...
            dict_key = f"{name}|tierlist_notes"
            if dict_key in CORRECTIONS:
                fellow['tierlist']['notes'] = CORRECTIONS[dict_key]
                corrections_applied += 1
...
def main():
    ...
    print(f"Processed {len(data)} fellows")
